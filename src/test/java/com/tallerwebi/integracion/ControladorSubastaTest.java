...
@ContextConfiguration(classes = {
        SpringWebTestConfig.class, HibernateTestConfig.class })
public class ControladorSubastaTest {
    ...
    public void siElUsuarioNoTieneSesionYEntraACrearSubastaDeberiaRedirigirALogin() throws Exception{
        MvcResult result = this.mockMvc.perform(get("/crear-subasta")).andExpect(status().is3xxRedirection())
    ...
    @Test
    public void siElUsuarioTieneSesionYEntraACrearSubastaDeberiaRedirigirACrearSubasta() throws Exception{
        MvcResult result = this.mockMvc.perform(get("/crear-subasta").sessionAttr("USUARIO_ID", 1L)).andExpect(status().isOk())
